...
function App() {
  ...
  return (
    <div
      className="min-h-screen bg-black text-white flex flex-col items-center pt-4 pb-20 relative"
      {...handlers}
    >
      <h1 className="text-3xl font-bold mb-4 text-center">
        NSFW Anime Slideshow
      ...
      {/* Image or Loader */}
      {images.length > 0 ? (
        <div className="relative flex items-center justify-center min-h-[75vh]">
          {loading && (
            <div className="absolute z-10">
              <div className="animate-spin h-16 w-16 rounded-full border-4 border-pink-500 border-t-transparent"></div>
            </div>
          )}
          <img
            src={images[index].file_url}
            ...
            className={`max-w-full max-h-[75vh] object-contain rounded-2xl shadow-lg transition-opacity duration-500 ${
      ...
      {/* Fixed Buttons */}
      <div className="fixed bottom-0 left-0 right-0 bg-black/80 py-3 flex justify-center gap-4 border-t border-gray-700 z-20">
        <button
          onClick={prevImage}
          className="bg-gray-800 px-4 py-2 rounded hover:bg-gray-700"
        >
